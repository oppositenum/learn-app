...
describe('VoiceCaptureButton', () => {
	it('places the transcript in editable text without submitting', async () => {
		...
		const status = wrapper.get('p[aria-live="polite"]')
		expect(status.classes()).toContain('text-base')
		expect(status.classes()).not.toContain('text-xs')
	...
	it('shows a microphone failure in a warm color at 16px', async () => {
		Object.defineProperty(navigator, 'mediaDevices', { configurable: true, value: { getUserMedia: vi.fn(async () => { throw new Error('denied') }) } })
		const wrapper = mount(VoiceCaptureButton, { props: { sessionId: 'session-1' } })
		await wrapper.get('button[aria-label="使用语音回答"]').trigger('click')
		await flushPromises()
		const alert = wrapper.get('[role="alert"]')
		expect(alert.text()).toBe('无法使用麦克风')
		expect(alert.classes()).toContain('text-base')
		expect(alert.classes()).toContain('text-amber-800')
		expect(alert.classes().filter((name) => name === 'text-xs' || name.startsWith('text-red'))).toEqual([])
		// The round button keeps its size.
		expect(wrapper.get('button').classes()).toContain('icon-button')
	})
})
