...
- The pending deployment branch contains a server-authoritative `ORIGINAL -> VARIANT -> ABSTRACT -> VERIFY -> COMPLETE` flow for complete `READY` task lineages. Every stage uses a versioned deterministic scorer, excludes every task already presented in the session, and requires a fresh independent task after help. The pending branch and repository-bundled content provide zero `READY` lineages, so this B4 mechanism remains dormant; content without a lineage stays on the legacy classroom path with its existing model correctness override. B3, B5, and B6 engineering implementation is complete, while B4 product activation and the model-override behavior transition have not met release conditions.
...
Integration gate baseline: top-level PASS >= 97, FAIL = 0, SKIP = 0, and TestIntegrationDatabaseConfiguredInCI must pass in CI.
